...
test("market discovery presents crypto and first-party paper predictions", async () => {
  const page = await source("src/app/(app)/market/page.tsx");
  const route = await source("src/app/api/prediction-markets/route.ts");
  const sync = await source("src/lib/prediction-sync.ts");
  assert.match(page, /Crypto & paper predictions/);
  assert.match(page, /\/api\/prediction-markets/);
  assert.match(page, /PAPER PREDICTIONS/);
  assert.match(page, /\/predictions\?marketId=/);
  assert.doesNotMatch(page, /Polymarket · research only/);
