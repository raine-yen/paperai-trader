...
function OrderRail({ symbol, price, initialSide, open, nonce, position, cash, onReview, onTraded }: { symbol: string; price: number; initialSide: Side; open: boolean; nonce: number; position: Position | null; cash: number; onReview: () => Promise<{ cash: number; position: Position | null; quote: Quote | null }>; onTraded: () => Promise<void> }) {
 ...
 useEffect(() => {
  ...
  <div className="vanta-order-content"><h2>{side === "buy" ? "Buy" : "Sell"} {symbol}</h2><div className="vanta-order-field"><Segment value={side} onChange={setSide} options={[["buy", "Buy"], ["sell", "Sell"]]} label="Order side" className="is-primary" /><Segment value={type} onChange={setType} options={[["market", "Market"], ["limit", "Limit"]]} label="Order type" /></div><div className="vanta-order-field"><Segment value={mode} onChange={setMode} options={[["dollars", "Dollars"], ["shares", "Shares"]]} label="Amount mode" /><label className="vanta-amount"><span>{mode === "dollars" ? "$" : "#"}</span><input ref={amountRef} aria-label={`Order amount in ${mode}`} type="number" min="0" step={mode === "dollars" ? "0.01" : "0.0001"} value={amount} onChange={(event) => setAmount(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && valid && !submitting) submit(); }} /></label><div className="vanta-quick-chips">{[0.25, 0.5, 0.75, 1].map((pct) => <button key={pct} type="button" disabled={!canSize} onClick={() => setSize(pct)} aria-label={pct === 1 ? `Use all available ${side === "sell" ? "shares" : "buying power"}` : `Quick size order to ${pct * 100}% of available ${side === "sell" ? "position" : "buying power"}`}>{pct === 1 ? "Max" : `${pct * 100}%`}</button>)}</div></div>{type === "limit" ? <label className="vanta-limit">Limit price<input aria-label="Limit price" type="number" min="0" step="0.01" value={limitPrice} onChange={(event) => setLimitPrice(event.target.value)} /></label> : null}<dl className="vanta-estimates"><Estimate label="Estimated price" value={estimatedPrice ? formatUSD(estimatedPrice) : "—"} /><Estimate label="Estimated shares" value={shares ? shares.toFixed(4) : "—"} /><Estimate label={side === "buy" ? "Estimated cost" : "Estimated proceeds"} value={notional ? formatUSD(notional) : "—"} /><Estimate label={side === "buy" ? "Buying power after" : "Shares after sale"} value={side === "buy" ? formatUSD(Math.max(0, cash - notional)) : Math.max(0, owned - shares).toFixed(4)} /></dl>{error ? <p role="alert" className="vanta-order-error">{error}</p> : null}{result ? <p role="status" className="vanta-order-result">{result}</p> : null}</div><footer className="vanta-order-footer"><button ref={confirmRef} type="button" className={cn("vanta-confirm-button", side === "sell" && "is-sell")} disabled={!valid || submitting} onClick={submit}>{submitting ? "Sending…" : `Confirm ${side === "buy" ? "Buy" : "Sell"}${notional ? ` · ${formatUSD(notional)}` : ""}`}</button><p className="vanta-order-note">Simulated trade — updates your paper account only.</p></footer></aside>
  );
}
