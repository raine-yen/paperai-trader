...
interface Quote { symbol?: string; price: number; prevClose: number | null; name?: string | null; exchange?: string | null; marketCap?: number | null; trailingPE?: number | null; volume?: number | null; averageVolume?: number | null; open?: number | null; dayHigh?: number | null; dayLow?: number | null; yearHigh?: number | null; yearLow?: number | null; change?: number | null; changePercent?: number | null; liveSource?: "anchor" | "ws" | "sim"; }
    ...
    const unsubscribe = feed.subscribe((liveQuotes) => {
      setQuotes((previous) => {
        ...
        for (const symbol of visibleSymbols) {
          ...
          if (live && existing) next.set(symbol, { ...existing, price: live.price, prevClose: live.prevClose, liveSource: live.source });
        }
        quotesRef.current = next;
        return next;
      });
  ...
  return (
    <section className="vanta-workspace">
      ...
      <main className="vanta-detail">
        ...
        <div className="vanta-price-block"><strong className="tabular-nums">{quote ? formatUSD(price) : "—"}</strong>{changePct != null ? <span className={cn(isUp ? "text-accent-green" : "text-accent-red")}><>{isUp ? <ArrowUpRight className="h-4 w-4" /> : <ArrowDownRight className="h-4 w-4" />}</>{formatUSD(Math.abs(change ?? 0))} ({formatPct(Math.abs(changePct))}) today</span> : <span>Awaiting paper quote</span>}{quote?.liveSource === "sim" ? <small>Simulated tick — not used for paper-order fills.</small> : null}</div>
...
function OrderRail({ symbol, price, initialSide, open, nonce, position, cash, onClose, onReview, onTraded }: { symbol: string; price: number; initialSide: Side; open: boolean; nonce: number; position: Position | null; cash: number; onOpen: (side: Side) => void; onClose: () => void; onReview: () => Promise<{ cash: number; position: Position | null; quote: Quote | null }>; onTraded: () => Promise<void> }) {
  const [stage, setStage] = useState<"configure" | "review">("configure"); const [side, setSide] = useState<Side>(initialSide); const [type, setType] = useState<OrderType>("market"); const [mode, setMode] = useState<Mode>("dollars"); const [amount, setAmount] = useState(""); const [limitPrice, setLimitPrice] = useState(price ? price.toFixed(2) : ""); const [reviewQuote, setReviewQuote] = useState<Quote | null>(null); const [submitting, setSubmitting] = useState(false); const [result, setResult] = useState(""); const [receipt, setReceipt] = useState<OrderReceipt | null>(null); const confirmRef = useRef<HTMLButtonElement | null>(null); const railRef = useRef<HTMLElement | null>(null); const amountRef = useRef<HTMLInputElement | null>(null);
 useEffect(() => { setSide(initialSide); setAmount(""); setResult(""); setReviewQuote(null); setStage("configure"); }, [initialSide, symbol, nonce]);
 useEffect(() => {
   if (!open) return;
  ...
  useEffect(() => {
    ...
    function onKey(event: KeyboardEvent) { if (event.key === "Escape") { setStage("configure"); setReviewQuote(null); onClose(); } }
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [stage, onClose]);
  useEffect(() => { setLimitPrice((current) => current || (price ? price.toFixed(2) : "")); }, [price]);
  const owned = Number(position?.qty ?? 0); const numberAmount = Number(amount) || 0; const displayedMarketPrice = stage === "review" ? Number(reviewQuote?.price ?? price) : price; const estimatedPrice = type === "limit" ? Number(limitPrice) || 0 : displayedMarketPrice; const shares = mode === "dollars" && estimatedPrice > 0 ? numberAmount / estimatedPrice : numberAmount; const notional = shares * estimatedPrice; const invalidLimit = type === "limit" && estimatedPrice <= 0; const valid = numberAmount > 0 && !invalidLimit;
  ...
  async function submit() { setSubmitting(true); try { const fresh = await onReview(); const freshPrice = type === "limit" ? Number(limitPrice) || 0 : Number(fresh.quote?.price ?? 0); const freshShares = mode === "dollars" && freshPrice > 0 ? numberAmount / freshPrice : numberAmount; const freshNotional = freshShares * freshPrice; const freshOwned = Number(fresh.position?.qty ?? 0); if (!freshShares || freshPrice <= 0 || invalidLimit) { setResult("A current quote and a valid order amount are required."); setStage("configure"); return; } if (side === "buy" && freshNotional > fresh.cash + 0.01) { setResult("This estimate is above your current simulated buying power. The server remains the final authority."); setStage("configure"); return; } if (side === "sell" && freshShares > freshOwned + 0.00001) { setResult(`Available to sell: ${freshOwned.toFixed(4)} shares. The server remains the final authority.`); setStage("configure"); return; } const response = await fetch("/api/trade", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ symbol, qty: Math.floor(freshShares * 10_000) / 10_000, side, type, limit_price: type === "limit" ? freshPrice : undefined }) }); const data = await response.json(); if (response.ok) { await onTraded(); const status = data.status === "filled" ? "filled" : "submitted" as OrderReceipt["status"]; setReceipt({ ok: true, title: status === "filled" ? "Order filled" : "Order submitted", detail: status === "filled" ? `${Math.floor(freshShares * 10_000) / 10_000} ${symbol} filled at ${formatUSD(Number(data.filled_avg_price ?? freshPrice))}.` : `${freshShares.toFixed(4)} ${symbol} is working. Fills appear in your portfolio shortly.`, status, side, symbol, amount: (Math.floor(freshShares * 10_000) / 10_000).toFixed(4) }); setAmount(""); setStage("configure"); setReviewQuote(null); onClose(); } else { setResult(data.error ?? "Paper order could not be submitted."); setStage("configure"); } } catch { setResult("Could not refresh the order details. Please try again."); setStage("configure"); } finally { setSubmitting(false); } }
  async function review() { setSubmitting(true); try { const fresh = await onReview(); const freshPrice = type === "limit" ? Number(limitPrice) || 0 : Number(fresh.quote?.price ?? 0); const freshShares = mode === "dollars" && freshPrice > 0 ? numberAmount / freshPrice : numberAmount; const freshNotional = freshShares * freshPrice; const freshOwned = Number(fresh.position?.qty ?? 0); if (!freshShares || freshPrice <= 0 || invalidLimit) { setResult("A current quote and a valid order amount are required."); return; } if (side === "buy" && freshNotional > fresh.cash + 0.01) { setResult("This estimate is above your current simulated buying power. Reduce the amount and try again."); return; } if (side === "sell" && freshShares > freshOwned + 0.00001) { setResult(`Available to sell: ${freshOwned.toFixed(4)} shares. Reduce the amount and try again.`); return; } setReviewQuote(fresh.quote); setStage("review"); } catch { setResult("Could not refresh the order details. Please try again."); } finally { setSubmitting(false); } }
  const error = invalidLimit ? "Enter a limit price above zero." : "";
  return <><aside ref={railRef} className={cn("vanta-order-rail", open && "is-open")} aria-label="Paper order ticket"><header><p>ACCOUNT</p><div><span>Buying power</span><strong className="tabular-nums">{formatUSD(cash)}</strong></div><small>Simulated funds only · no real money</small><button type="button" className="vanta-rail-close" aria-label="Close order ticket" onClick={onClose}><X className="h-4 w-4" /></button></header><div className="vanta-order-content"><h2>{side === "buy" ? "Buy" : "Sell"} {symbol}</h2><Segment value={side} onChange={setSide} options={[["buy", "Buy"], ["sell", "Sell"]]} label="Order side" /><Segment value={type} onChange={setType} options={[["market", "Market"], ["limit", "Limit"]]} label="Order type" /><Segment value={mode} onChange={setMode} options={[["dollars", "Dollars"], ["shares", "Shares"]]} label="Amount mode" /><label className="vanta-amount"><span>{mode === "dollars" ? "$" : "#"}</span><input ref={amountRef} aria-label={`Order amount in ${mode}`} type="number" min="0" step={mode === "dollars" ? "0.01" : "0.0001"} value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="0" /><button onClick={setMax}>Max</button></label>{type === "limit" ? <label className="vanta-limit">Limit price<input aria-label="Limit price" type="number" min="0" step="0.01" value={limitPrice} onChange={(event) => setLimitPrice(event.target.value)} /></label> : null}<dl className="vanta-estimates"><Estimate label="Estimated price" value={estimatedPrice ? formatUSD(estimatedPrice) : "—"} /><Estimate label="Estimated shares" value={shares ? shares.toFixed(4) : "—"} /><Estimate label={side === "buy" ? "Estimated cost" : "Estimated proceeds"} value={notional ? formatUSD(notional) : "—"} /><Estimate label={side === "buy" ? "Buying power after" : "Shares after sale"} value={side === "buy" ? formatUSD(Math.max(0, cash - notional)) : Math.max(0, owned - shares).toFixed(4)} /></dl>{error ? <p role="alert" className="vanta-order-error">{error}</p> : null}{result ? <p role="status" className="vanta-order-result">{result}</p> : null}<button className="vanta-review-button" disabled={!valid || submitting} onClick={review}>Review paper order</button><p className="vanta-order-note">No order is sent until you confirm the simulated order on the next step.</p></div>{stage === "review" ? <div className="vanta-review-scrim" role="presentation" onClick={() => { setStage("configure"); setReviewQuote(null); onClose(); }}><div className="vanta-review-dialog" role="dialog" aria-modal="true" aria-labelledby="review-title" onClick={(event) => event.stopPropagation()}><p>PAPER ORDER REVIEW</p><h2 id="review-title">Confirm simulated {side}?</h2><dl><Estimate label="Order" value={`${side === "buy" ? "Buy" : "Sell"} ${shares.toFixed(4)} ${symbol}`} /><Estimate label="Estimated total" value={formatUSD(notional)} /><Estimate label="Type" value={type} /></dl><p>Illustrative only. This will update your paper account, never a real brokerage account.</p><div><button onClick={() => { setStage("configure"); setReviewQuote(null); onClose(); }}>Edit</button><button ref={confirmRef} className="vanta-review-button" onClick={submit} disabled={submitting}>{submitting ? "Submitting…" : "Confirm paper trade"}</button></div></div></div> : null}</aside>{receipt ? <OrderSuccessOverlay receipt={receipt} onDismiss={() => setReceipt(null)} /> : null}</>;
}
