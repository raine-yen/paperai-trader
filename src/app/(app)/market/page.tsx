...
function MarketWorkspace({ symbol, initialSide, initialQuote, position, cash, fetchAccount, fetchQuotes, onBack, onTraded }: { symbol: string; initialSide: Side; initialQuote: Quote | null; position: Position | null; cash: number; fetchAccount: () => Promise<AccountSnapshot | null>; fetchQuotes: (symbols: string[], force?: boolean) => Promise<Map<string, Quote>>; onBack: () => void; onTraded: () => Promise<void> }) {
  ...
  return (
    <section className="vanta-workspace">
      ...
      <OrderRail symbol={symbol} price={price} initialSide={initialSide} position={position} cash={cash} onReview={refreshOrderState} onTraded={onTraded} />
    </section>
  );
}

function OrderRail({ symbol, price, initialSide, position, cash, onReview, onTraded }: { symbol: string; price: number; initialSide: Side; position: Position | null; cash: number; onReview: () => Promise<{ cash: number; position: Position | null; quote: Quote | null }>; onTraded: () => Promise<void> }) {
  const [side, setSide] = useState<Side>(initialSide); const [type, setType] = useState<OrderType>("market"); const [mode, setMode] = useState<Mode>("dollars"); const [amount, setAmount] = useState(""); const [limitPrice, setLimitPrice] = useState(price ? price.toFixed(2) : ""); const [reviewing, setReviewing] = useState(false); const [reviewQuote, setReviewQuote] = useState<Quote | null>(null); const [submitting, setSubmitting] = useState(false); const [result, setResult] = useState("");
  useEffect(() => { setSide(initialSide); setAmount(""); setResult(""); setReviewQuote(null); }, [initialSide, symbol]);
  useEffect(() => { setLimitPrice((current) => current || (price ? price.toFixed(2) : "")); }, [price]);
  const owned = Number(position?.qty ?? 0); const numberAmount = Number(amount) || 0; const displayedMarketPrice = reviewing ? Number(reviewQuote?.price ?? price) : price; const estimatedPrice = type === "limit" ? Number(limitPrice) || 0 : displayedMarketPrice; const shares = mode === "dollars" && estimatedPrice > 0 ? numberAmount / estimatedPrice : numberAmount; const notional = shares * estimatedPrice; const invalidLimit = type === "limit" && estimatedPrice <= 0; const valid = numberAmount > 0 && !invalidLimit;
